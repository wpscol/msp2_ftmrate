...
int main(int argc, char* argv[]) {
  ...
  cmd.AddValue("nodePause", "Maximum time station waits in newly selected position (s) - only for RWPM mobility type",
               nodePause);
  ...
  if (mobilityModel == "Distance") {
    ...
    mobility.SetMobilityModel("ns3::RandomWaypointMobilityModel", "Speed", StringValue(ssSpeed.str()), "Pause",
  ...
  for (uint32_t j = 0; j < wifiStaNodes.GetN(); ++j) {
    InstallTrafficGenerator(wifiStaNodes.Get(j), wifiApNode.Get(0), portNumber++, applicationDataRate, packetSize);
  }
  ...
  csvOutput << mobilityModel << ',' << wifiManagerName << ',' << delta << ',' << interval << ',' << velocity << ','
            << distance << ",," << nWifi << ',' << nWifiReal << ',' << RngSeedManager::GetRun() << ',' << totalThr
            << std::endl;
...
void ChangePower(Ptr<Node> staNode, uint8_t powerLevel) {
  ...
  devicePath << "/NodeList/" << staNode->GetId()
